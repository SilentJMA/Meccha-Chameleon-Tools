#!/usr/bin/env python3
"""HyperVision engine — fast TCP bridge + fallback terrain scan."""
import json, math, os, socket, struct, time
from typing import List, Tuple, Optional, Dict

BRIDGE_HOST = "127.0.0.1"
BRIDGE_PORT = 47654

# Cache for bridge alive status (avoid blocking connect every call)
_bridge_cache_ok = False
_bridge_cache_time = 0.0
_BRIDGE_CACHE_TTL = 3.0  # re-check every 3s max


def _send(cmd: str, payload: dict = None, timeout: float = 5) -> dict:
    msg = json.dumps({
        "type": cmd, "request_id": f"{os.urandom(8).hex()}{int(time.time())}",
        "timestamp_utc": int(time.time()), "payload": payload or {},
    }) + "\n"
    try:
...
def _ping_fast() -> bool:
    """Non-blocking-ish ping with 0.3s timeout."""
    global _bridge_cache_ok, _bridge_cache_time
    now = time.time()
    if now - _bridge_cache_time < _BRIDGE_CACHE_TTL:
        return _bridge_cache_ok
    _bridge_cache_time = now
    try:
        s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        s.settimeout(0.3)
        s.connect((BRIDGE_HOST, BRIDGE_PORT))
...
def bridge_start_hv(tx, ty, tz, px, py, pz, quality=1):
    return _send("start_hypervision", {
        "target": [tx, ty, tz], "player": [px, py, pz], "quality": quality,
    }, timeout=60)


def bridge_update_hv(tx, ty, tz, px, py, pz):
    return _send("update_hypervision", {
        "target": [tx, ty, tz], "player": [px, py, pz],
    }, timeout=10)


def bridge_stop_hv():
    return _send("stop_hypervision", timeout=10)
...
class HyperVisionEngine:
    """Caches exposure + path data. No blocking IO in constructor."""

    def __init__(self, config):
        self.config = config
        self.terrain_segments: List[Tuple] = []
        self.last_terrain_time = 0.0
    ...
    def check_bridge(self) -> bool:
        now = time.time()
        if now - self._last_bridge_check < _BRIDGE_CACHE_TTL:
            return self._bridge_alive
        self._last_bridge_check = now
        self._bridge_alive = _ping_fast()
        return self._bridge_alive
    ...
    def refresh_terrain(self, cam_pos, force=False) -> List[Tuple]:
        ...
        if self._bridge_alive and cam_pos:
    ...
    def update_targets(self, players: list, cam_pos):
        """Round-robin scan one target per call if bridge alive + enabled."""
        if not self._bridge_alive or not cam_pos:
            return {}
        if not self.config.hypervision_enabled:
            return {}
        ...
        now = time.time()
        self._scan_counter = (self._scan_counter + 1) % len(enemies)
        ...
        if r.get("success"):
            ...
            self.target_cache[eid] = {"cloud": cloud, "paths": paths,
                                      "time": now, "pos": ep}
        return dict(self.target_cache)
    ...
    def get_render_data(self, camera, screen_w, screen_h):
        def _w2s(wp, cam, sw, sh):
            ...
            aspect = sw / sh
            ndc_x = vy / (vx * tan_hfov)
            ndc_y = vz / (vx * tan_hfov / aspect)
            return ((1 + ndc_x) * sw / 2, (1 - ndc_y) * sh / 2)

        path_lines, exposure_dots = [], []
