...
def _hello_line(instance_guid_hex, token_hex):
    return json.dumps({
        "type": "hello",
        "bootstrap_protocol": 1,
        "instance_id": instance_guid_hex,
        "token": token_hex,
    }, separators=(",", ":"))

def bridge_request(command, payload=None, timeout=30, hello=None):
    if _bridge_port is None:
        return {"success": False}
    msg = (command if isinstance(command, str) and command.startswith("{") else
           json.dumps({"type": command, "request_id": f"{os.urandom(8).hex()}{int(time.time())}",
                       "timestamp_utc": int(time.time()), "payload": payload or {}},
                      separators=(",", ":"))) + "\n"
    s = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
    s.settimeout(timeout)
    try:
        s.connect((BRIDGE_HOST, _bridge_port))
        if hello:
            s.sendall((hello + "\n").encode("utf-8"))
            buf = b""
            while b"\n" not in buf:
                ...
                if not chunk:
                    break
                buf += chunk
            hello_resp = json.loads(buf.split(b"\n")[0])
            if not (hello_resp.get("success") and hello_resp.get("stage") == "hello"):
                return {"success": False}
