...
|----------|-------------|
| **ESP** ⚠️ WIP | Dot / 2D Box / Corner Box / Skeleton overlays, names, role labels (Hunter/Survivor), distance, snap lines, enemy-only filter, visible/not-visible coloring, invincible detection, per-role toggles, team filter, distance scaling, Draw All |
...
# Meccha Chameleon Tools (full tools with ESP, Aimbot, Camouflage)
pyinstaller --clean meccha_chameleon_tools.spec

# MecchaCamouflage (standalone camouflage-only)
pyinstaller --clean MecchaCamouflage.spec

# Camouflage Lighter (tkinter standalone, no PyQt5 needed)
cd camouflage-lighter
pip install -r requirements.txt
